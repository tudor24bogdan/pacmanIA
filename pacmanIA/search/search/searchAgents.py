...
def foodHeuristic(state, problem):
    ...
    distances_to_food = [util.manhattanDistance(position, food) for food in uneaten]

    min_distance_to_food = min(distances_to_food)

    max_distance_between_food = 0
    for i in range(len(uneaten)):
        for j in range(i + 1, len(uneaten)):
            dist = util.manhattanDistance(uneaten[i], uneaten[j])
            max_distance_between_food = max(max_distance_between_food, dist)

    heuristic_value = min_distance_to_food + max_distance_between_food
